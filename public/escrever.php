...
include_once './observer/LogObserver.php'; // Incluindo o observador
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Criando a conexão com a base de dados (se necessário)
    $facade = new EmailFacade($mysqli);

    // Criando o observador de log
    $logObserver = new LogObserver('log.txt'); // Ou pode passar o caminho completo

    // Registrando o observador na Facade
    ...
    try {
        ...
        $envio = date('Y-m-d H:i:s'); // Gera o timestamp atual no formato correto
        $modificado = 0;
